...
try:
    idx = next(i for i,f in enumerate(files) if os.path.basename(f) == '20191012_1030.bin')
except:
    idx = -1
    print('not found')
...
cv2.imwrite('conv_pred_3.png', rainfall_shade(pred[-1]))
cv2.imwrite('conv_label_3.png', rainfall_shade(data[-1]))