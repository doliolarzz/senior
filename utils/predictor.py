...
def prepare_testing(data, model, weight=1, stride=120, config=None):

    t, height, width = data.shape
    assert t == config['IN_LEN'] + global_config['OUT_TARGET_LEN']
    input = data[:config['IN_LEN']]
    label = data[-global_config['OUT_TARGET_LEN']:]

    assert global_config['OUT_TARGET_LEN'] % config['OUT_LEN'] == 0

    n_h = int((height - global_config['IMG_SIZE'])/global_config['STRIDE']) + 1
    n_w = int((width - global_config['IMG_SIZE'])/global_config['STRIDE']) + 1

    pred = np.zeros((global_config['OUT_TARGET_LEN'], height, width), dtype=np.float32)
    vals = np.zeros((config['OUT_LEN'], height, width), dtype=np.float32)
    counts = np.zeros((config['OUT_LEN'], height, width), dtype=np.float32)
    for i in range(int(global_config['OUT_TARGET_LEN']/config['OUT_LEN'])):
        vals[:] = 0
        counts[:] = 0
        for h in range(n_h):
            for w in range(n_w):
                ...
                vals[:, h_start:h_end, w_start:w_end] += predict(input[:, None, h_start:h_end, w_start:w_end], model, config=config)[:, 0] * weight
                counts[:, h_start:h_end, w_start:w_end] += weight

        for h in range(n_h):
            h_start = h*global_config['STRIDE']
            h_end = h*global_config['STRIDE'] + global_config['IMG_SIZE']
            vals[:, h_start:h_end, -global_config['IMG_SIZE']:] += predict(input[:, None, h_start:h_end, -global_config['IMG_SIZE']:], model, config=config)[:, 0] * weight
            counts[:, h_start:h_end, -global_config['IMG_SIZE']:] += weight

        for w in range(n_w):
            w_start = w*global_config['STRIDE']
            w_end = w*global_config['STRIDE'] + global_config['IMG_SIZE']
            vals[:, -global_config['IMG_SIZE']:, w_start:w_end] += predict(input[:, None, -global_config['IMG_SIZE']:, w_start:w_end], model, config=config)[:, 0] * weight
            counts[:, -global_config['IMG_SIZE']:, w_start:w_end] += weight
        
        vals[:, -global_config['IMG_SIZE']:, -global_config['IMG_SIZE']:] += predict(input[:, None, -global_config['IMG_SIZE']:, -global_config['IMG_SIZE']:], model, config=config)[:, 0] * weight
        counts[:, -global_config['IMG_SIZE']:, -global_config['IMG_SIZE']:] += weight
